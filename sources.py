...
class ProxyCheckSource(Source):
    ...
    def fetch(self, ip: str) -> dict:
        ...
        if data.get("status") != "ok":
            raise ValueError(
                f"proxychceck.io returned status={data.get('status')} for {ip}"
            )
...
class IpWhoIsSource(Source):
    ...
    def fetch(self, ip: str) -> dict:
        ...
        if not data.get("success", True):
            raise ValueError(
                f"ipwho.is returned success=False ({data.get('message')}) for {ip}"
            )
...
class IPQualityScoreSource(Source):
    ...
    def fetch(self, ip: str) -> dict:
        ...
        if not data.get("success", True):
            raise ValueError(
                f"IPQS returned success=false ({data.get('message')}) for {ip}"
            )
